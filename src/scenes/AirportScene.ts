...
export class AirportScene extends Phaser.Scene {
  ...
  // ----- Plane icon helper -----
  /**
   * Thin scene-bound wrapper around the shared `makePlaneIcon` (see
   * `src/ui/PlaneIcon.ts`). Kept as an instance method so the existing
   * call sites (parked layer, visitor layer, takeoff anim, etc.) don't
   * need to thread `this` explicitly through every invocation. */
